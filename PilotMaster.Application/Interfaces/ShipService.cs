...
public class ShipService : IShipService
{
    ...
    private static void Validate(Ship ship)
    {
        if (string.IsNullOrWhiteSpace(ship.Name))
            throw new ValidationException("Nome do navio é obrigatório.");

        if (ship.GRT <= 0)
            throw new ValidationException("GRT deve ser maior que zero.");

        if (ship.Draft <= 0)
            throw new ValidationException("Draft inválido.");
